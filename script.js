...
function fillFormFields(card) {
    ...
    if (sportType === "Indoor") {
        const indoorSports = ["Chess", "Badminton", "Carrom Board"];
        ...
        const outdoorSports = [ "Tennis", "Cricket", "Foot Ball", "Hockey"];
        outdoorSports.forEach(outdoorSport => {
            const option = new Option(outdoorSport, outdoorSport);
            sportDropdown.appendChild(option);
        });
        if (outdoorSports.length > 0) {
            sportDropdown.value = outdoorSports[0];
        }
    }
...
const updateCard = (card, name, age, gender, sportType, sport, profilePicture) => {
    ...
    cardInfo.forEach(info => {
        ...
        if (element) {
            element.innerText = info.content;
        } else {
